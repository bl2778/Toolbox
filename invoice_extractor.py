...
def extract_text_for_gpt(pdf_path: str) -> str:
    """Extract text from PDF for GPT processing (currently disabled)"""
    full_text = ""
...
class InvoiceExtractor(object):
    def __init__(self, path):
        self.file = path if os.path.isfile(path) else None

    def _load_data(self):
        if self.file and os.path.splitext(self.file)[1] == '.pdf':
            with pb.open(self.file) as pdf:
                page = pdf.pages[0]
            ...
            for col in range(COLS):
                ...
                while cnt <= COLS:
                    ...
                    if p0 and p1 and p2 and p3:
                        rects.append(((df.columns[col], df.index[row]),
                                      (df.columns[cnt], df.index[row]),
                                      (df.columns[col], df.index[row+1]),
                                      (df.columns[cnt], df.index[row+1])))
                        break
                    else:
                        cnt += 1
        return rects
    ...
    def _find_outer(self, k, words):
        ...
        for pos, text in words.items():
            if re.search(r'发票$', text):  # 发票名称
                ...
                date = ''.join(re.findall(r'[0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日', text))
                df.loc[0, '开票日期'] = date
    ...
    def _find_inner(self, k, words, groups, groups2, free_zone_flag=False):
        ...
        text = [word for _, word in sort_words]
        context = ''.join(text)
        ...
        if '购买方' in context or '销售方' in context:
            ...
            for pos, txt in group_context.items():
                if '名称' in txt:
                    name = re.sub(r'名称:', '', txt)
                    df.loc[0, prefix+'名称'] = name
                elif '纳税人识别号' in txt:
                    tax_man_id = re.sub(r'纳税人识别号:', '', txt)
                    df.loc[0, prefix+'纳税人识别号'] = tax_man_id
                elif '地址、电话' in txt:
                    addr = re.sub(r'地址、电话:', '', txt)
                    df.loc[0, prefix+'地址电话'] = addr
                elif '开户行及账号' in txt:
                    account = re.sub(r'开户行及账号:', '', txt)
            ...
            target_index = self._index_of_y(x, same_rects_k)
            target_k = same_rects_k[target_index]
            words2 = groups2[target_k]
            context2 = [v for _, v in words2.items()]
            context2 = ''.join(context2)
            df.loc[0, '密码区'] = context2
            ...
            group_context = ''.join([w for _, w in group_words.items()])
            ...
            # 若当前单元格的文字无法匹配到任何特定标签，我们考虑是否处于开票项目明细"自由区"
            if free_zone_flag:
            ...
            if len(same_rects_k) == 8:
                ...
                for kk in same_rects_k:
                    words2 = groups2[kk]
                    words2 = sorted(words2.items(), key=lambda x: x[0]) if words2 else None
                    if words2 and len(words2) > 0:
                        key = words2[0][1]
                        val = [word[1] for word in words2[1:]]
    ...
    def extract(self):
        ...
        if not data:
            return pd.DataFrame()  # 返回空表，或也可以抛出异常
        ...
        for k, words in word_groups2.items():
            if k[0] == 'OUT':
                ...
                df_item, free_zone_flag = self._find_inner(k, words, word_groups, word_groups2, free_zone_flag)
